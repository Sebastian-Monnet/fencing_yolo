...
class Viewer:
    def __init__(self, vid_path, box_list_list, left_list, right_list):
...
for ind in clip_inds:
    modes = ['box_list_lists/', 'left_lists/', 'right_lists/']
    vid_path = 'clips/' + str(ind) + '.mp4'
    filenames = ['extracted_data/' + mode + str(ind) + '.pt' for mode in modes]
    box_list_list, left_list, right_list = [pickle.load(open(filenames[i], 'rb')) for i in range(3)]
    b = Viewer(vid_path, box_list_list, left_list, right_list)
